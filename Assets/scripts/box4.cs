...
public class box4 : MonoBehaviour
{
    ...
    public static bool isFinished4 = false;
    ...
    void Start()
    {
        aObject = box.transform.Find("balls3").gameObject;
    ...
    IEnumerator BallFlow()
    {
        ...
        if (levelCount == 2)
        {
            for (int i = 1; i < (clones.Length + 1); i++)
            {
                ...
                speedTemp = liquidSpawner.room4Speeds[i - 1];
                speedTemp2 = liquidSpawner.room1Speeds[i - 1];
                speedTemp3 = liquidSpawner.room2Speeds[i - 1];
                speedTemp4 = liquidSpawner.room3Speeds[i - 1];
                if (i == 1)
                {
