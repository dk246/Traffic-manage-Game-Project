...
public class box3 : MonoBehaviour
{
    ...
    public static bool isFinished3 = false;
    ...
    void Start()
    {
        aObject = box.transform.Find("balls4").gameObject;
        //animator = GetComponent<Animator>();
        currentBalls = 15;
        //animator.updateMode = AnimatorUpdateMode.UnscaledTime;
    }
    ...
    public void StartBox3()
    {
        stillIn = true ;
        collecter.isCame = false ;
        currentBalls = 15;
        isFinished3 = false;
    ...
    IEnumerator BallFlow()
    {
        ...
        if (levelCount == 3)
        {
            for (int i = 1; i < (clones.Length + 1); i++)
            {
                ...
                speedTemp = liquidSpawner.room3Speeds[i - 1];
                speedTemp2 = liquidSpawner.room4Speeds[i - 1];
                speedTemp3 = liquidSpawner.room1Speeds[i - 1];
                speedTemp4 = liquidSpawner.room2Speeds[i - 1];
                if (i == 1)
                {
        ...
        else if (levelCount == 1)
        {
            for (int i = 1; i < (clones.Length + 1); i++)
            {
                ...
                //yield return new WaitForSeconds(speedTemp3);
